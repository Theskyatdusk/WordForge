...
export function checkAchievements(
  ...
  const check = (achievement: Achievement, condition: boolean) => {
    if (condition && !allUnlocked.has(achievement.id)) {
      allUnlocked.add(achievement.id);
      newAchievements.push(achievement);
    }
  };

  // First step: any study session
  check(ACHIEVEMENTS[0], stats.totalStudied > 0);

  // Streak achievements
  check(ACHIEVEMENTS[1], stats.streak.current >= 3);
  check(ACHIEVEMENTS[2], stats.streak.current >= 7);
  check(ACHIEVEMENTS[3], stats.streak.current >= 30);

  // Studied count achievements
  check(ACHIEVEMENTS[4], stats.totalStudied >= 50);
  check(ACHIEVEMENTS[5], stats.totalStudied >= 100);
  check(ACHIEVEMENTS[6], stats.totalStudied >= 500);

  // Mastered count achievements
  check(ACHIEVEMENTS[7], stats.masteredCount >= 10);
  check(ACHIEVEMENTS[8], stats.masteredCount >= 50);
  check(ACHIEVEMENTS[9], stats.masteredCount >= 100);

  // Wordbook achievements
  check(ACHIEVEMENTS[10], stats.wordbook.length >= 10);

  // Perfect day: 100% accuracy with >= 10 words studied today
  ...
  check(ACHIEVEMENTS[11], !!isPerfect);

  // Quiz count achievements (total sessions)
  check(ACHIEVEMENTS[12], stats.totalSessions >= 50);

  // Level achievements
  check(ACHIEVEMENTS[13], stats.level.level >= 5);
  check(ACHIEVEMENTS[14], stats.level.level >= 10);
